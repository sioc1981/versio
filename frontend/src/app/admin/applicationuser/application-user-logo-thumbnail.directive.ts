 ...
 *
 * Based on William KOZA (oKoza)'s thumbnail.directive.ts; add gif and svg+xml.
 */
@Directive({
    selector: '[logoThumbnail]',
    exportAs: 'logoThumbnail'
})
export class ApplicationUserLogoThumbnailDirective implements OnInit {

    @Input('logoThumbnail') fileItem: FileItem;
    ...
    ngOnInit() {
        ...
        if (this.fileItem.file.type.indexOf('image/jpeg') !== 0 &&
            this.fileItem.file.type.indexOf('image/gif') !== 0 &&
            this.fileItem.file.type.indexOf('image/png') !== 0 &&
            this.fileItem.file.type.indexOf('image/svg+xml') !== 0) {
            return;
        } else {
